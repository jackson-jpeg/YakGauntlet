...
export class Corner3RightScene extends Phaser.Scene {
  // Basketball
  ...
  // Hoop geometry
  private readonly BACKBOARD_X = 140;
  ...
  create(): void {
    ...
    // UI
    this.ui = createSceneUI(this, 4, 'Misses');
    ...
    // Side indicator
    this.add.text(GAME_WIDTH - 25, GAME_HEIGHT / 2, 'RIGHT\nCORNER', {
      fontSize: '12px',
      fontFamily: YAK_FONTS.title,
      color: '#ff6b35',
      stroke: '#000000',
  ...
  private createCourt(): void {
    ...
    const logoText = this.add.text(GAME_WIDTH / 2, 720, 'YAK', {
      fontSize: '28px',
      fontFamily: YAK_FONTS.title,
      color: '#e74c3c',
  ...
  private drawRim(): void {
    ...
    // Rim - elliptical view from angle
  ...
  private createBasketball(): void {
    ...
    // Highlight
    const highlight = this.add.circle(-7, -7, 7, 0xffffff, 0.35);
    this.ball.add(highlight);

    // Small highlight
    const smallHighlight = this.add.circle(-4, -12, 3, 0xffffff, 0.5);
    this.ball.add(smallHighlight);
  }
  ...
  update(): void {
    ...
    // Apply spin effect (Magnus effect lite)
  ...
  private checkBackboardCollision(): void {
    ...
    if (this.ballState.x - this.BALL_RADIUS < backboardRight &&
        this.ballState.x + this.BALL_RADIUS > backboardLeft &&
        this.ballState.y > backboardTop &&
        this.ballState.y < backboardBottom &&
        this.ballState.vx < 0) {

      this.ballState.x = backboardRight + this.BALL_RADIUS;
  ...
  private handleRimBounce(rimX: number, rimY: number, dist: number): void {
    ...
    // Rim rattle text on multiple bounces
  ...
  private checkHoopSuccess(): void {
    // Ball must be going down and pass through the hoop opening
    ...
    // Confirm basket when ball exits below the net
  ...
  private checkBounds(): void {
    // Check if ball missed completely
    ...
    // Too many rim bounces without success
  ...
  private handleSuccess(): void {
    ...
    for (let i = 0; i < 20; i++) {
      const particle = this.add.circle(
        this.rimCenterX + (Math.random() - 0.5) * 60,
        this.HOOP_Y + 30,
        Math.random() * 5 + 2,
        [0x4ade80, 0xfbbf24, 0xff6b35, 0x60a5fa][Math.floor(Math.random() * 4)],
        0.9
      ).setDepth(150);
